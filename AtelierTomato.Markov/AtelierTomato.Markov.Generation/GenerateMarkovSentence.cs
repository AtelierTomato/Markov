...
namespace AtelierTomato.Markov.Generation
{
	public class GenerateMarkovSentence
	{
		...
		private async Task<Sentence?> GetNextSentence(List<string> prevList, List<string> previousIDs, ISentenceFilter filter) => await sentenceAccess.ReadNextRandomSentence(prevList, previousIDs, filter);

		private async Task<Sentence?> GetFirstSentence(ISentenceFilter filter) => await sentenceAccess.ReadRandomSentence(filter);
	}
}
