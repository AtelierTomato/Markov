...
namespace AtelierTomato.Markov.Storage.Sqlite
{
	public class SqliteLocationGroupPermissionAccess : ILocationGroupPermissionAccess
	{
		...
		public async Task DeleteLocationFromLocationGroup(ulong ID, IObjectOID location)
		{
		...
		public async Task<LocationGroupPermission?> ReadLocationGroupPermission(ulong ID, IObjectOID location) => (await ReadLocationGroupPermissionHierarchyForGroup(ID, location)).FirstOrDefault();
		public async Task<IEnumerable<LocationGroupPermission>> ReadLocationGroupPermissionHierarchyForGroup(ulong ID, IObjectOID location)
		{
			await using var connection = new SqliteConnection(options.ConnectionString);
			connection.Open();
...
@location || ':' LIKE {nameof(LocationGroupPermission.Location)} || ':%'
ORDER BY LENGTH({nameof(LocationGroupPermission.Location)}) DESC
",
			new
			{
				id = ID,
		...
		public async Task<IEnumerable<LocationGroupPermission>> ReadLocationGroupPermissionRangeByBaseLocation(IObjectOID location)
		{
			await using var connection = new SqliteConnection(options.ConnectionString);
			connection.Open();
...
",
		...
		public async Task<IEnumerable<LocationGroupPermission>> ReadLocationGroupPermissionHierarchy(IObjectOID location)
		{
			await using var connection = new SqliteConnection(options.ConnectionString);
			connection.Open();
			...
			var result = await connection.QueryAsync<LocationGroupPermissionRow>($@"
SELECT {nameof(LocationGroupPermission.ID)}, {nameof(LocationGroupPermission.Location)}, {nameof(LocationGroupPermission.Permissions)} FROM {nameof(LocationGroupPermission)}
WHERE @location || ':' LIKE {nameof(LocationGroupPermission.Location)} || ':%'
ORDER BY LENGTH({nameof(LocationGroupPermission.Location)}) DESC
",
			new
			{
		...
		public async Task<IEnumerable<LocationGroupPermission>> ReadLocationGroupPermissionRangeByID(ulong ID)
		{
			await using var connection = new SqliteConnection(options.ConnectionString);
			connection.Open();
...
",
		...
		public async Task<LocationGroupPermissionType> ReadLocationGroupPermissionsForOwner(ulong ID, AuthorOID author)
		{
			await using var connection = new SqliteConnection(options.ConnectionString);
			connection.Open();
