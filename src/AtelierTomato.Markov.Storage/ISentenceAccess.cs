...
namespace AtelierTomato.Markov.Storage
{
	...
	{
		/// <summary>
		/// Read a random <see cref="Sentence"/> that matches the given <paramref name="filter"/>,  are allowed to access from the <paramref name="origin"/>, and includes the given <paramref name="keyword"/>.
		/// </summary>
		...
		/// <param name="origin">The location as an <see cref="IObjectOID"/> the generate request is coming from. Used to determine what <see cref="Sentence"/>s are allowed.</param>
		/// <returns></returns>
		Task<Sentence?> ReadRandomSentence(SentenceFilter filter, string? keyword = null, IObjectOID? origin = null);

		/// <summary>
		/// Reads an <paramref name="amount"/> of random Sentences that match to the given <paramref name="filter"/>, are not in <paramref name="previousIDs"/>, are allowed to access from the <paramref name="origin"/>, and match to the <paramref name="prevList"/>, prioritizing those that contain the given <paramref name="keyword"/>.
		/// </summary>
		...
		/// <param name="origin">The location as an <see cref="IObjectOID"/> the generate request is coming from. Used to determine what <see cref="Sentence"/>s are allowed.</param>
		/// <returns></returns>
		Task<IEnumerable<Sentence>> ReadNextRandomSentences(int amount, List<string> prevList, List<IObjectOID> previousIDs, SentenceFilter filter, string? keyword = null, IObjectOID? origin = null);
