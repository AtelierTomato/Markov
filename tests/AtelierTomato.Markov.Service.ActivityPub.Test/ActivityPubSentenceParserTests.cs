...
{
	public class ActivityPubSentenceParserTests
	{
		[Fact]
		public void ParseEmojiTest()
		{
			var options = new SentenceParserOptions();
			var target = new ActivityPubSentenceParser(Options.Create(options));

			IReadOnlyList<ActivityPubEmoji> emojis = [new ActivityPubEmoji("madou", "_", "_", true), new ActivityPubEmoji("mikanScreech", "_", "_", true)];
			var result = target.ParseIntoSentenceTexts([], emojis, ":madou: 1 2 3 4 :mikanScreech: :gregor:");

			result.Should().ContainSingle().And.Contain("e:madou: 1 2 3 4 e:mikanScreech: :gregor :");
		}
		[Fact]
		public void ParseMentionTest()
		{
			var options = new SentenceParserOptions();
			var target = new ActivityPubSentenceParser(Options.Create(options));

			IReadOnlyList<ActivityPubMention> mentions = [new ActivityPubMention("1234", "kelsey", "https://here.goto.tomato.pink/@kelsey", "kelsey@goto.tomato.pink"), new ActivityPubMention("123", "greg", "https://website.com/@greg", "greg")];
			var result = target.ParseIntoSentenceTexts(mentions, [], "@kelsey@goto.tomato.pink @kelsey@here.goto.tomato.pink @kelsey@tomato.pink @greg@website.com hi i am saying hi to you");

			result.Should().ContainSingle().And.Contain("kelsey kelsey kelsey greg hi i am saying hi to you");
		}

		[Fact]
		public void ParsePollTest()
		{
			var options = new SentenceParserOptions();
			var target = new ActivityPubSentenceParser(Options.Create(options));

			var poll = new ActivityPubPoll("lolpoll", DateTimeOffset.Now, true, true, 2, true, [
				new("this is my awesome title", 1),
				new("NEVER select this options it is bad! please do not select it!", 1)
			], []);
			var result = target.ParseIntoSentenceTexts([], [], "hi please look at my poll lmao", poll);

			result.Should().BeEquivalentTo([
				"hi please look at my poll lmao",
				"this is my awesome title",
				"NEVER select this options it is bad !",
				"please do not select it !"
			]);
		}

		[Fact]
		public void ParseSpoilerTextTest()
		{
			var options = new SentenceParserOptions();
			var target = new ActivityPubSentenceParser(Options.Create(options));

			var result = target.ParseIntoSentenceTexts([], [], "i just ate a really yummy burger", null, "please don't look omg it's so embarrassing");

			result.Should().BeEquivalentTo([
				"please don 't look omg it 's so embarrassing",
				"i just ate a really yummy burger"
			]);
		}

		[Fact]
		public void ParseAllActivityPubStuffTest()
		{
			var options = new SentenceParserOptions();
			var target = new ActivityPubSentenceParser(Options.Create(options));
			...
			IReadOnlyList<ActivityPubMention> mentions = [new ActivityPubMention("1234", "kelsey", "https://here.goto.tomato.pink/@kelsey", "kelsey@goto.tomato.pink"), new ActivityPubMention("123", "greg", "https://website.com/@greg", "greg")];
			var poll = new ActivityPubPoll("lolpoll", DateTimeOffset.Now, true, true, 2, true, [
				new(":madou: the awesome funny face from madou monogatari!", 1),
				new(":mikanScreech: the hungry girl from pripara!", 1)
			], []);

			var result = target.ParseIntoSentenceTexts(mentions, emojis, "@greg@website.com @kelsey@goto.tomato.pink you MIGHT be interested in this ;)", poll, "trigger warning: showdown between the greatest emojis Of All Time");

			result.Should().BeEquivalentTo([
				"trigger warning : showdown between the greatest emojis Of All Time",
				"greg kelsey you MIGHT be interested in this ; )",
				"e:madou: the awesome funny face from madou monogatari !",
				"e:mikanScreech: the hungry girl from pripara !"
			]);
		}

		// Below are tests copied from SentenceParserTests, but using the ActivityPubSentenceParser. All should pass or those that should not pass have been removed.
		[Theory]
